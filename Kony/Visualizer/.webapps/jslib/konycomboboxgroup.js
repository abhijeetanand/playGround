...
$KW.ComboBox =
{
    ...
    render: function(comboboxModel, context){
		...
		if(!comboboxModel.buiskin) comboboxModel.buiskin = comboboxModel.blockeduiskin;
    ...
    generateList: function(comboboxModel, data, context){
		...
		if(!comboboxModel.view  || comboboxModel.view == 'native'){
			if (data.length > IndexJL) {			
					...
					comboboxModel.selectedkey = key ? key : ((data.length==1)?data[IndexJL][IndexJL]:null); //First item will be selected if there is no selectedkey
		...
		htmlString = "<select "+ (kony.appinit.isIE7 || kony.appinit.isIE8 ? " onchange='$KW.ComboBox.eventHandler(arguments[0],this)'": " ") + $KW.Utils.getBaseHtml(comboboxModel, context) + "class='" + computedSkin + "'" + (comboboxModel.disabled ? " disabled='true'" : "") + " style='"+$KW.skins.getMarginSkin(comboboxModel, context)+ $KW.skins.getPaddingSkin(comboboxModel, context) +"'>";			
		for (var i = IndexJL; i < (data.length); i++) {
			if (data[i][IndexJL] != null && data[i][1 + IndexJL] != null) {
				...
				var ariaString	= $KU.getAccessibilityValues(comboboxModel, data[i][2+IndexJL], data[i][IndexJL]);
					htmlString += "<option  " + ariaString + " value='" + data[i][IndexJL] + "' " + selected + ">" + data[i][1 + IndexJL] + "</option>";
			}
		}			
		htmlString += "</select>";
