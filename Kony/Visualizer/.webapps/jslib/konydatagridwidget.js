...
$KW.DataGrid = {
	...
	create : function(gridModel, context) {
		...
		var rowdata = gridModel.data || [null];	
	...
	headerBuilder : function(columnModel, gridModel) {
		...
		var padding = $KW.skins.getPaddingSkin(gridModel);
		...
		for(var i = IndexJL; i < columnModel.length; i++) {
			...
			headerStr = temp + (colInfo.columnheadertext || "") + '</th>';  
			...
			for (var i = IndexJL; i < rowdata.length; i++) {
				...
				for (var j = 0; j < noofcolumns; j++) {
					...
					if (type == "text") {
						...
						temp = textTmpl.replace(/\{text\}/g, text);
	...
	modifyContent: function(gridModel, dataArray, action, index, columnID){ // index starts at 1
		...
		if (gridModel)  {
			...
			if (holder) {											
				...
				this.toggleVisibilty(gridModel, gridModel.data);
