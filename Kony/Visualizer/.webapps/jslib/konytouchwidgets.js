...
$KW.touch.konyScroller.prototype = 
{
	...
	_checkDOMChanges: function () 
	{
		...
		if (this.moved || this.animating ||
			(this.scrollerW == scroller.offsetWidth * 1 && this.scrollerH == scroller.offsetHeight * 1)) return;
	...
	refresh: function()
	{
		...
		that.scrollerW = Math.round(that.minScrollX  + that.scroller.offsetWidth + that.maxBiasX);
		//that.scrollerH = Math.round(that.scroller.offsetHeight + that.minScrollY);
		that.scrollerH = Math.round(that.minScrollY + that.scroller.offsetHeight +  + that.maxBiasY);
	...
	extendTouchMove: function(touch)
	{
		...
		if (newX > this.minScrollX || newX < this.maxScrollX)
			newX = this.options.bounce ? this.x + (deltaX / 2) : newX >= this.minScrollX || this.maxScrollX >= 0 ? this.minScrollX : this.maxScrollX;
	...
	extendTouchEnd: function(touch)
	{
		...
		if (duration < 300)
		{
			...
			momentumX = newPosX ? this._momentum(newPosX - this.startX, duration, -this.x, (this.maxScrollX < 0 ? this.scrollerW - this.wrapperW + this.x - this.minScrollX : 0), this.options.bounce ? this.wrapperW : 0) : momentumX;
			momentumY = newPosY ? this._momentum(newPosY - this.startY, duration, -this.y, (this.maxScrollY < 0 ? this.scrollerH - this.wrapperH + this.y - this.minScrollY : 0), this.options.bounce ? this.wrapperH : 0) : momentumY;
	...
	// kony.setFocus()
	scrollToElement: function (el, time, diff) 
	{
		if (!el) return;
		...
		if(this.scrollerH > this.wrapperH)
		{
			...
			if(((Math.abs(this.y) + this.wrapperH) < (el.offsetTop+el.offsetHeight)) || (el.offsetTop < Math.abs(this.y)) || $KU.isWindowsPhone){
				if($KU.isAndroid && diff && pos.top != that.maxScrollY){				
					pos.top = that.y - diff;					
				}
...
$KW.touch.gesture.prototype = 
{
	...
	executeCallback: function(curTarget,dir)
	{
		if (this.callback) 
		{
			var gestureInfoObj = {};
			...
			var coords = $KW.Utils.getOffset(this.widget);
			...
			gestureInfoObj.gesturePosition = $KU.getgesturePosition(touch.pageX,touch.pageY,this.widget.clientWidth,this.widget.clientHeight,coords.left,coords.top);
			
			if(gestureInfoObj.gestureType == 2)
			...
			gestureInfoObj.widgetWidth = this.widget.clientWidth;
			gestureInfoObj.widgetHeight = this.widget.clientHeight;
...
$KW.touch.Drag = function (widget, dragableElement, containerElement, dragEvent, moveElement){
    this.widget = widget;
