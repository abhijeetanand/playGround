...
$KW.Segment = {
            ...
            case "viewtype":
                if(propertyValue != oldPropertyValue && element) {
                    ...
                    if(propertyValue == constants.SEGUI_VIEW_TYPE_PAGEVIEW) {
                        ...
                        element = parentNode.childNodes[0];
    ...
    generateSegment: function(segmentModel, context) {
        ...
        var visibility = (!segmentModel.isvisible || segData.length <= IndexJL) ? "hide" : ""; //Hide the segment if there is no data
        var skin = segmentModel.widgetskin || segmentModel.skin;
		var widgetSkin = skin ? skin + "grp" : "";
        var computedSkin = visibility + " " + widgetSkin;
		var margin = $KW.skins.getMarginSkin(segmentModel, context);
        var padding = $KW.skins.getPaddingSkin(segmentModel);
		var scrollingevents;
		var isSLW = segmentModel.screenLevelWidget && (segmentModel.parent == segmentModel.pf);
		if(isSLW)
        ...
        if(pageView) 
		{
            ...
            htmlString += "name='touchcontainer_Segment' class='kstripcontainer " + computedSkin + " " + rowSkin + "'";
		...
		else 
		{
			if(isSLW)
			{
				htmlString += "<div  id='" + segmentModel.pf+"_"+ segmentModel.id + "_scroller' class='scrollerX  " + computedSkin + "' kwidgettype='KScrollBox' name='touchcontainer_KScroller' swipeDirection ='vertical' style='" + margin + "'>" + 
                            "<div id='" + segmentModel.pf+"_"+ segmentModel.id + "_scrollee' class='form_scrollee' kwidgettype='KTouchscrollee'>";
				htmlString += (scrollingevents && scrollingevents.onPull) ? $KU.getPullString() : "" ;
			}

            htmlString += "<div" + $KW.Utils.getBaseHtml(segmentModel, context);
			htmlString += "class='" + (!isSLW ? computedSkin : "") + "' style='" + (!isSLW ? margin : "") + padding + "'>";
        ...
        if(segData.length > IndexJL) {
            ...
            if(!pageView && segmentModel.hasSections === false) {
                htmlString += "<ul style='list-style:none' class='";
                htmlString += widgetSkin+"grp";
                htmlString += "'>";
            }
        ...
        if(pageView) {
			...
			if(isSLW){
				htmlString += (scrollingevents && scrollingevents.onPush) ? $KU.getPushString() : "" ;
				 htmlString += "</div></div>";
			}
        }
    ...
    renderPageFooter: function(segmentModel) {
        ...
        var visibility = (!segmentModel.isvisible || segData.length <= 1) ? "hide" : "";
        ...
        var footer = "<div class='ktable kwt100 " + visibility + "' style='" + (segmentModel.needpageindicator ? "" : "display:none;") + $KW.Segment.getPageFooterStyle(segmentModel) +"' id='" + segmentModel.pf + "_" + segmentModel.id + "_footer'>" + this.generatePageFooter(segmentModel, segData) + "</div>";
        return footer;
    },
    ...
    generateRows: function(model, context, data, liExists, rowIndex, secIndex) {
        ...
        for(var i = IndexJL; i < data.length; i++) {
            ...
            if(section || section === "") { //Here the new section is getting started // (i == IndexJL || sectionRows == i - IndexJL)
                if(section instanceof Object) { //section is defined with a sectionTemplate
                    ...
                    if(headertemplate && $KU.getkeys(section).length > 0) {
                        ...
                        sectionHtml = this.renderHBox(clonedHeaderTemplate, context, section, model);
                        if(sectionHtml === ""){
                            sectionHtml = " ";
			            }
                ...
                if(sectionHtml) { //Start a new section in case of setdata
                    if(i == IndexJL) {
                        htmlString += "<ul style='list-style:none' class='" + rowSkin + "'>";
                    } else if(sectionRows == i - IndexJL) {
                        secIndex++;
                        rowIndex = IndexJL;
                        htmlString += "</ul><ul style='list-style:none' class='" + rowSkin + "'>";
                    }
            ...
            if(boxModel && !boxModel.pf) {
                window[boxModel.id] = boxModel;
                _konyConstNS.Form2.addHeaderorFooter.call(boxModel, boxModel); //TODO:: If this line is really required ?????
            }
            ...
            rowHtml = (clonedBoxModel.wType=="HBox" || clonedBoxModel.orientation=="horizontal") ? this.renderHBox(clonedBoxModel, context, data[i], model) : this.renderVBox(clonedBoxModel, context, data[i], model);
    ...
    generatePageFooter: function(segmentModel, data) {
        ...
        if(data.length > IndexJL) {
            ...
            for(var i = IndexJL; i < data.length; i++)
            {
				var isAriaHidden = data[i].accessibilityConfig.a11yHidden;
    ...
    modifyContent: function(model, data, action, rowIndex, secIndex) {
        ...
        if(action == "addat" || action == "adddataat" || action == "removeat" || action == "addall") {
            if(!model.hasSections) {
        ...
        if(model.viewtype == constants.SEGUI_VIEW_TYPE_PAGEVIEW) {
            ...
            if(model.needpageindicator) {
                ...
                $KU.toggleVisibilty(footerHolderNode, model.data, model);
            }
        }
        ...
        if(map) {
            ...
            if(data[IndexJL] instanceof Array) {                
                ...
                for(var i = IndexJL; i < data.length; i++) { 
					...
					if(sectionData && typeof sectionData != "object" &&  typeof sectionData !== "number" && sectionData.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
						newdata[index]["sectionLabel"] = eval(sectionData);
					else if(sectionData instanceof Object) {
						newdata[index]["sectionLabel"] = {};
						for(var sKey in sectionData) {
							if(newmap[sKey]) {
								var sValue = sectionData[sKey];
								if(sValue && typeof sValue != "object" && typeof sValue !== "number" && sValue.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
									newdata[index]["sectionLabel"][newmap[sKey]] = eval(sValue);
								else
                    ...
                    for(var j = IndexJL; j < innerData.length; j++, index++) {
                        ...
                        for(var k = 0; k < newkeys.length; k++) {
                            ...
                            if(value && typeof value != "object" &&  typeof value !== "number" && value.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
                                newdata[index][newmap[newkeys[k]]] = eval(value);
                            else
                                newdata[index][newmap[newkeys[k]]] = value;
                        }
                        innerData[j]["metainfo"] && (newdata[index]["metainfo"] = innerData[j]["metainfo"]);
			...
			innerData[j]["accessibilityConfig"] && (newdata[index]["accessibilityConfig"] = innerData[j]["accessibilityConfig"]);
                ...
                for(var i = IndexJL; i < data.length; i++) {
                    ...
                    for(var j = 0; j < newkeys.length; j++) 
                    {
                        ...
                        if(value && typeof value != "object" &&  typeof value !== "number" && value.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
                            newdata[i][newmap[newkeys[j]]] = eval(value);
                        else
                            newdata[i][newmap[newkeys[j]]] = value;
                    }
                    if(data[i]["metainfo"]) {
                        newdata[i]["metainfo"] = $KU.cloneObj(data[i]["metainfo"]);
                        for(var j = 0; j < newkeys.length; j++) {
                            if(newdata[i]["metainfo"][newkeys[j]]) {
                                newdata[i]["metainfo"][newmap[newkeys[j]]] = data[i]["metainfo"][newkeys[j]];
    ...
    renderHBox : function(widgetModel, context, rowData, segmentModel) {
        ...
        htmlString += "<div class = 'ktable " + computedSkin + "'" + $KW.Utils.getBaseHtml(widgetModel, context, null, rowData.accessibilityConfig) + "style='table-layout:fixed;" + style + "'><div class = 'krow " + layoutDirection + " kwt100'>";
    ...
    renderVBox : function(widgetModel, context, rowData, segmentModel) {
        //Do not need this as we can access childModel by widgetModel.childModel.id
        //var parentModel = kony.model.getWidgetModel(widgetModel.pf, context.tabpaneID);
        var topLevelBox = context.topLevelBox;
        var layoutDirection = $KW.skins.getWidgetAlignmentSkin(widgetModel);        
        ...
        if(topLevelBox) {
            htmlString += "<div  class = 'kwt100 kbasemargin ktable'" +  $KW.Utils.getBaseHtml(widgetModel, context, null, rowData.accessibilityConfig) + " style='table-layout:fixed;'>";
        }
        else {
    ...
    adjustAlreadySelectedProperties: function(widgetModel, action, secIndex, rowIndex) {
        if(action != "addall") {
			...
			if(widgetModel.selectedrowindices) {               
                widgetModel.selectedrowindices = this.adjustSelectedRows(widgetModel.selectedrowindices, action, secIndex, rowIndex);
				widgetModel.selectedrowindices && (widgetModel.selectedrowindex = widgetModel.selectedrowindices[widgetModel.selectedrowindices.length - 1]);
            }
            this.setSelectedItemsAndIndices(widgetModel);
        }
    },
