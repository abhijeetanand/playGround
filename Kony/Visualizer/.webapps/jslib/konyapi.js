	...
	showLoadingScreen: function() 
	{
		...
		var setLoadingPosition = function(event) 
		{
			...
			else 
			{
				...
				$KU.changea11yDynamicElement("loading indicator in progress");
				// Append to body
				//document.body.appendChild(divTag);
			}
		};
		...
		if(position == constants.LOADING_SCREEN_POSITION_FULL_SCREEN || isBlocked)
		{
			...
			if($KG.relativeScroll && !$KU.isIDevice)
		...
		if(!loadingDiv && $KG["relativeScroll"]){
			...
			kony.events.addEventListener(divTag, 'touchstart', preventScroll);
			kony.events.addEventListener(divTag, 'touchmove', preventScroll);
		}
	},
