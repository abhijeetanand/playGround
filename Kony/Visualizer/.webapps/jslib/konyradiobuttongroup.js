...
$KW.RadioButtonGroup = {
    ...
    render: function(radiobuttonModel, context){
        ...
        var skinArray = $KW.skins.getSplitSkinsBetweenWidgetAndParentDivForCBRB(radiobuttonModel, context);
		var css = (context.ispercent === false ?  "" : skinArray[2]) + ((!$KW.Utils.isWidgetVisible(radiobuttonModel, context) || data.length <= 0) ? " hide" : "");        
        var widgetSkin = radiobuttonModel.skin || "";
		var htmlString = "<div" + $KW.Utils.getBaseHtml(radiobuttonModel, context) + "class = '" + css + " " + widgetSkin + "' style='"+$KW.skins.getMarginSkin(radiobuttonModel, context) + $KW.skins.getPaddingSkin(radiobuttonModel) + (context.ispercent === false ?  "display:inline-block" : "") + "'>";
		// (radiobuttonModel.itemorientation != constants.RADIOGROUP_ITEM_ORIENTATION_VERTICAL)
           //tmlString += "<p mode='wrap' class='" + skinArray[0] + "'>";
				...
				if (data[i][IndexJL] != null && data[i][1+IndexJL] != null) {
					...
					if(radiobuttonModel.viewtype == "customview" && !kony.appinit.isIE8)
					{
						if (radiobuttonModel.itemorientation == "vertical") {
							htmlString += "<div style = 'height: auto;'  class='" + skinArray[0] + "'>";
						}
						else {
					...
					else
					{
						...
						if (ariaString.indexOf('aria-label') < 0)
							ariaString += " aria-label='" + data[i][1+IndexJL] + "' ";
						...
						htmlString += "<span aria-hidden='true' class='middlecenteralign'>" +  data[i][1+IndexJL] + "</span>";
