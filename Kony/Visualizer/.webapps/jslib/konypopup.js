$KW.Popup = {
 ...
 render: function(popupModel){
		...
		*/
        ...
        var cwtSkin = $KW.skins.getMarPadAdjustedContainerWeightSkin(popupModel) + " " + (popupModel.skin || "");
        var htmlString = "";
		...
		htmlString += "<form id='" + popupId + "' class='" + cwtSkin + "' style='z-index:9;position: absolute;" + $KW.skins.getPaddingSkin(popupModel)
		...
		// ?? Assume a popup is opened n times n closed, the zIndex is increased n times!!
        $KW.Popup.zIndex = $KW.Popup.zIndex || 10;
        popup.style.zIndex = ++$KW.Popup.zIndex;
    ...
    show: function(popupModel){
		...
		var containerHeight = parseInt(popupModel.containerheight, 10);
		...
		if(popupModel.containerheight && !$KU.isBlackBerryNTH)
		...
		if(!popupModel.context && !popupModel.enableScroll)  
		{
	...
	adjustPosition: function(popupModel, popupelem){
		...
		var top = Math.floor((100 - Math.floor((popupelem.offsetHeight * 100) / (window.innerHeight || document.body.clientHeight))) / 2) + "%";
		var left = Math.floor((100 - Math.floor((popupelem.offsetWidth * 100) / (window.innerWidth || document.body.clientWidth))) / 2) + "%";
	...
	domremove: function(id)
	{
		...
		if (popupelem) 
		{
			...
			if(/*$KU.iOS4 &&*/ $KG["relativeScroll"] && !popupModel.context)
			{
				if(opacity == 1)
				{
					var header = document.getElementById("header_container");
    ...
    applyTransition: function(model, popupelem, endTrans){
		...
		model.height = Math.floor(popupHeight + (Math.abs(popupelem.parentNode.offsetHeight - popupHeight) / 2)); //height to travel for transitions
		...
		if(transtype == "bottomCenter"){
			model.height = popupelem.parentNode.offsetHeight + popupHeight;
		}
	...
	setAnchorPosition: function(model, popupelem, endTrans)
	{
        ...
        if (context && context.widget) 
		{
            ...
            if (elem) {
				...
				if(model.enableScroll)
					popupelem.style.marginTop = 0;				
            }
        }
	...
	adjustPopupDimensions: function(model, popupelem)
	{
		...
		if($KG["relativeScroll"] && model.ismodal)
		{
