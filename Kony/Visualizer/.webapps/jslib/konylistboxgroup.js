...
$KW.ListBox = {
   ...
   render: function(listboxModel, context){
    ...
    generateList: function(listboxModel, data, context){
		...
		{
            htmlString = "<select" + $KW.Utils.getBaseHtml(listboxModel, context) + "class='" + computedSkin + "' " + (listboxModel.disabled ? "disabled='true' " : "") + multiple + size + " style='" + $KW.skins.getMarginSkin(listboxModel, context)+ $KW.skins.getPaddingSkin(listboxModel, context) + "'>";
    ...
    generateOptions: function(listboxModel, data){
        ...
        for (var i = IndexJL; i < data.length; i++) {
            if (data[i][IndexJL] != null) {
                ...
                htmlString += "<option value=\"" + data[i][IndexJL] + "\" " + selected + ariaString + ">" + data[i][1 + IndexJL] + "</option>";
            }
        }
        return htmlString;
    },
