...
$KW.Calendar =
{
    ...
    render : function(calendarModel, context){
        ...
        var marginPaddding = $KW.skins.getMarginSkin(calendarModel, context) + $KW.skins.getPaddingSkin(calendarModel);
        if (calendarModel.viewtype === constants.CALENDAR_VIEW_TYPE_GRID_POPUP || calendarModel.viewtype == "default") {
            ...
            htmlString += " style='" + ($KU.isAndroid ? "position:relative;" : "") + ("text-align:" + cAlign + "; vertical-align: middle;") + marginPaddding + (context.ispercent === false ?  "display:inline-block" : "") + "'>";
    ...
    eventHandler: function(eventObject, target){
        ...
        if (!model) return;
            ...
            if (target.className.indexOf('-kony-w-c-next-nav') > -1){
                ...
                var a11yCalendarUpdate  = calendarModel.accessibilityconfig ? (calendarModel.accessibilityconfig.a11yLabel+' '+calendarModel.date) : ("Calendar "+calendarModel.date);
...
$KW.Calendar.__dp = {	
	...
	navigatorTemplate : function(model, id) {
        ...
        if(kony.appinit.isWindowsPhone){
			...
			for (var i = 0; i < months.length; i++) {
	            ...
	            str += "<div w-type='Calendar'   style='display:table-row;width:100%;height:50%'>";
	            for (var j = 0; j < list.length; j++) {
	                str += "<div w-type='Calendar'  style='display:table-cell;width:15%;text-align:center;vertical-align:middle;font-size:smaller' class='-kony-w-c-navigator-month' k-w-c-hold='" + ( (i * 6) + (j + 1) ) + "' k-w-c-id='" + id + "' kwidgettype='Calendar'" + WidgetContext + " >" +
	                            list[j].substring(0,3) +
	                        "</div>";
	            }
	            str += "</div>";
	        ...
	        for (i = 1; i <= 4; i++) { // 4 is number of rows of years We want to display.
				...
				// SO here 4 Columns * 4 Rows = 16 years will be displayed.
	                if((counter > 1900 && counter < 2099 )|| counter == 2099 || counter == 1900){
						str += "<div w-type='Calendar'  style='display:table-cell;width:25%;text-align:center;vertical-align:middle;font-size:smaller;visibility:visible;' class='-kony-w-c-navigator-year' k-w-c-hold='" + counter + "' k-w-c-id='" + id + "' kwidgettype='Calendar' " + WidgetContext + " >" +
	                       counter  +
	                    "</div>";		
					}else{
						str += "<div w-type='Calendar'  style='display:table-cell;width:25%;text-align:center;vertical-align:middle;font-size:smaller;visibility:hidden;' class='-kony-w-c-navigator-year' k-w-c-hold='" + counter + "' k-w-c-id='" + id + "' kwidgettype='Calendar' " + WidgetContext + " >" +
	                       counter  +
	                    "</div>";	
					}
	                counter++;
	...
	*/
		...
		for(var j = 0; j < monthData.length; j++){
			...
			if(typeof unit == "number"){ // next and prev month dates
				...
				if( (unit[2] == 0 || unit[2] == 6) && (isWeek == false) ){
					...
					element.innerHTML = $KG["i18nArray"]["platform.calendar."+unit[0]] ? $KG["i18nArray"]["platform.calendar."+unit[0]] : unit[0];
					element.removeAttribute('kwidgettype');
				}else{
	...
	show: function (calendarModel, ms, flag, calDIV ){
        ...
        for (var i = 0; i < calendarModel.noofmonths; i++){
