kony.events = {
	...
	getEventHandler: function (eventObject) 
	{
	    ...
	    if(!$KG["relativeScroll"] && eventObject.type == "click" && ($KU.isTouchSupported || $KU.isPointerSupported) && $KG["targetWidget"] && ($KG["moved"] || ($KG["targetWidget"] && targetWidget != $KG["targetWidget"]))) 
		{
			// Reset moved params
			$KG["targetWidget"] = "";
			$KG["moved"] = false;
	...
	stopPropagation: function(eventObject){        
	 ...
	 ontouchstartHandler: function(e) {
		if(($KU.isIE ? e : e.changedTouches) && !$KG["relativeScroll"])
		{
			...
			$KG["moved"] = false;
			if($KU.isIE){ //Disable click event when the user scrolls the page
	...
	ontouchmoveHandler: function(e) {
		if(!$KG["relativeScroll"]){
			if($KU.isIE){ //In windows phone or tablet, pointer move gets fired when the screen is touched.
				var touch = e.touches && e.touches[0] || e;
	...
	windowOrientationChange: function()
	{
		...
		if(orientation != undefined && orientation == $KG["__orientation"] && event && event.type == 'resize'){	
			...
			kony.events.resizeTimeoutId = setTimeout(function(){
				if($KG.activeInput && !$KG["relativeScroll"] && winNewHeight > $KG['__viewportHeight']){
	...
	executeBoxEvent: function(wModel, rowdata, containerModel, isSection) {
        if(rowdata && containerModel) { //Then widgetModel is inside a container widget
            ...
            while(parentModel) {
                ...
                parentModel = templateModel[parentModel.parent];
                if(!parentModel) return false;
            }
        } else {
			var formId = wModel.pf;
			var form = $KG.allforms[formId] || $KG.__currentForm;
	        var pModel = wModel.parent && form && form[wModel.parent];
			while(pModel) {			
				if ( pModel.onclick || (formId == pModel.parent)) {
					if(pModel.onclick){
	...
	browserback: {
		...
		handleBrowserBackEvent : function(){
			...
			if($KG["__idletimeout"] && $KG["__idletimeout"]["enabled"]) 
	         ...
	         if (window.location.hash && kony.events.browserback.currentHash && window.location.hash !== kony.events.browserback.currentHash){
				...
				if(previousFormModel && previousFormModel.wType == "Form") {
	    ...
	    updateURLWithLocation : function(formID){
	        ...
	        if (formID){
				if(typeof cvox !== "undefined" && window.location.hash != this.HASH_PREFIX + formID ){
                    try{
