...
$KW.Switch = {
    ...
    render: function(switchModel, context){
		...
		switchModel.selectedindex = switchModel.selectedindex || IndexJL+1;		
		if(kony.appinit.isIE9 || kony.appinit.isIE8){
			var htmlString = "<div" + $KW.Utils.getBaseHtml(switchModel, context) + "class='ktable kwt100 switch " + computedSkin + "' style='"+marginpadding+";position:relative'>" + 
				"<div class='krow kwt100 switchRow'>" + 
	            	"<div class='kcell kwt50 " + (switchModel.skin ? (switchModel.skin + "on"): "on") + "' id='" + switchModel.id + "_on'>" + lText + "</div>" + 
	            	"<div class='switchThumb thumb kwt50' id='" + switchModel.id + "_thumb' style='left:" + (["40%","0%"][switchModel.selectedindex]) + "''></div>" + 
	            	"<div class='kcell kwt50 " + (switchModel.skin ? (switchModel.skin + "off"): "off") + "' id='" + switchModel.id + "_off'>" + rText + "</div>" + 
	            "</div>" + 
	        	"</div>";
		}else{
                    if($KU.isIOS7){
                        ...
                        var offStyle    = [
                            ...
                            "-webkit-border-radius:15.5px",
                            ""].join(";");
                            var htmlString = "<div role='option' aria-selected='" + [ 'true' , 'false' ][switchModel.selectedIndex] + "' " + substituteAccessAttr + $KW.Utils.getBaseHtml(switchModel, context) + "class='switch " + computedSkin + "' style='" + marginpadding + switchStyle + "'>" + 
                            "<div class='" + (switchModel.skin ? (switchModel.skin + "on"): "on") + "' id='" + switchModel.id + "_on' style='" + onStyle + "'>" + lText + "</div>" + 
                            "<div class='thumb' id='" + switchModel.id + "_thumb' style='" +
                            thumbStyle + "'>" + "<span style='" +
                            thumbSpanStyle + "'></span></div>" +
                            "<div class=' " + (switchModel.skin ? (switchModel.skin + "off"): "off") + "' id='" + switchModel.id + "_off' style='" +
                            offStyle + "'>" + rText + "</div>" +
                            "</div>";
                    } else {
			var htmlString = "<div role='option' aria-selected='" + [ 'true' , 'false' ][switchModel.selectedIndex] + "' " + substituteAccessAttr + $KW.Utils.getBaseHtml(switchModel, context) + "class='switch " + computedSkin + "' style='"+marginpadding+"'>" + 
	            "<div aria-hidden='true' class='" + (switchModel.skin ? (switchModel.skin + "on"): "on") + "' id='" + switchModel.id + "_on'>" + lText + "</div>" + 
	            "<div aria-hidden='true' class='thumb' id='" + switchModel.id + "_thumb'> <span></span></div>" +
	            "<div aria-hidden='true' class=' " + (switchModel.skin ? (switchModel.skin + "off"): "off") + "' id='" + switchModel.id + "_off'>" + rText + "</div>" + 
	        	"</div>";
		    }
		}
        return htmlString;
	...
	adjustWidth: function(switchModel, switchNode, attachEvent){
		if(!(kony.appinit.isIE9 || kony.appinit.isIE8)){
                    if($KU.isIOS7) {
                        ...
                        switchModel.width = Math.floor(switchNode.clientWidth);
                        switchModel.swidth = switchModel.width + (switchNode.clientWidth % 2) + 6;
                        if (attachEvent == true) {
                            kony.events.addEventListener(switchNode, "touchstart", function(model) {
                                return function(e){
                                kony.events.preventDefault(e);
                                $KW.Switch.toggleSwitch(model);
                            }}(switchModel));
                        }
			...
			var paddingLeft = Math.ceil(switchNode.offsetWidth*(parseInt(switchNode.style.paddingLeft)*.01));
			var paddingRight = Math.ceil(switchNode.offsetWidth*(parseInt(switchNode.style.paddingRight)*.01));
			switchModel.width = (Math.floor((switchNode.clientWidth)-(paddingLeft+paddingRight)) / 2);
			switchModel.swidth = switchModel.width + (switchNode.clientWidth % 2) + 6;
			...
			switchNode.children[1].childNodes[1].style.width = switchModel.width + "px"; //Span in Thumb div
			if (attachEvent == true) {
                           if (typeof cvox !== "undefined") { // if screen reader is active in android
                               kony.events.addEventListener(switchNode, "click", function(model) {
                                   return function(e) {
                                       kony.events.preventDefault(e);
                                       $KW.Switch.toggleSwitch(model);
                                   }}(switchModel));
	...
	toggleSwitch: function(model){		
		...
		if (switchDiv && kdisabled != "true")
		{
		 	...
		 	if (model.selectedindex == IndexJL) {
		 		...
		 		if(kony.appinit.isIE9 || kony.appinit.isIE8){
					...
					if(! model.paddingInPixel)	paddingLeft = Math.ceil(switchDiv.offsetWidth*(parseInt(switchDiv.style.paddingLeft)*.02));
		 			...
		 			switchDiv.children[0].style[vendor + 'Transform'] = translateOpen + (-(model.width + paddingLeft+ 8)) + "px,0" + translateClose;
		 			switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + (-model.width) + "px,0" + translateClose;
		 			switchDiv.children[2].style[vendor + 'Transform'] = translateOpen + (-model.width) + "px,0" + translateClose;
					}
				}
			}
		 	else {
			...
			$KW.Switch.addA11YAttribute( model, switchDiv, true );                           // add left || right-side-text
	...
	adjustSwitch: function(model, switchDiv, trans){        
        ...
        if (model.selectedindex == IndexJL) {
            switchDiv.children[0].style[vendor + 'Transform'] = translateOpen + "0,0" + translateClose;
            $KU.isIOS7 && (switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + "1.5px,0" + translateClose) ||
            (switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + "0,0" + translateClose);
            ...
            switchDiv.children[0].style[vendor + 'Transform'] = translateOpen + (-(model.width + paddingLeft +6)) + "px,0" + translateClose;
            $KU.isIOS7 && (switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + 21.5 + "px,0" + translateClose) ||
            (switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + (-model.width) + "px,0" + translateClose);
