...
$KI.i18n = 
{
	...
	{ 
		...
		if($KG[lsKey]){
			...
			if(strRetrieved)
				existingBundle = eval('(' + strRetrieved + ')');
		}		
	...
	isresourcebundlepresent: function(locale)
	{
		...
		else
		{
			...
			var lsKey = $KG["appid"] + "_" + locale;
			var strRetrieved = (localStorage && localStorage.getItem(lsKey)) || $KG[lsKey];
			if (strRetrieved)
				return true;
			else
				return false;
	...
	*/
	...
	translateFormModel: function(formModel)
	{
		for(var child in formModel)
		{
			...
			if(widgetModel.wType == "Switch")
			{
				if(widgetModel.i18n_leftSideText && widgetModel.i18n_leftSideText.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
				{
					widgetModel.leftSideText = eval(widgetModel.i18n_leftSideText) || "";
				}				
				if(widgetModel.i18n_rightSideText && widgetModel.i18n_rightSideText.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
				{
					widgetModel.rightSideText = eval(widgetModel.i18n_rightSideText) || "";
				}
			}
			else if(widgetModel.wType == "TabPane")
			{
				...
				{
					var tabpageModel = widgetModel[tabpages[i]];
					if(tabpageModel.i18n_tabname && tabpageModel.i18n_tabname.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
					{
						tabpageModel.tabname = eval(tabpageModel.i18n_tabname) || "";
					}			
				}
			}
			else*/ if($KU.inArray($KU.translatableWidgets, widgetModel.wType, true))
			{
				if(widgetModel.i18n_text /* && widgetModel.i18n_text.toLowerCase().indexOf("i18n.getlocalizedstring") != -1 */)
				{
					...
					widgetModel.text = eval(widgetModel.i18n_text) || "";
					widgetModel.canUpdateUI = true;
				}
				// @removed
				/*
				if(widgetModel.wType == "TextField" && widgetModel.i18n_placeholder && widgetModel.i18n_placeholdert.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
				{
					widgetModel.placeholder = eval(widgetModel.i18n_placeholder) || "";
				}
				*/
			}
		}
	...
	getI18nTitle: function(formModel)
	{
		...
		if(formModel.i18n_title /* && formModel.i18n_title.toLowerCase().indexOf("i18n.getlocalizedstring") != -18 */)
