...
$KW.HBox =
{
    ...
    render: function(boxModel, context){
        ...
        if (!topLevel) {
            //the below code only for SPA, optimized for desktop.
           // #ifdef SPA
		...
		computedSkin += !$KW.Utils.isWidgetVisible(boxModel, context) ? " hide" : ""; 
            var boxstyle = /* $KW.Utils.getNormalImageStyle(boxModel) + */ " table-layout:fixed;" + $KW.skins.getMarginSkin(boxModel, context) + $KW.skins.getPaddingSkin(boxModel);
        ...
        if(boxModel.children){
			for (var i = 0; i < boxModel.children.length; i++) {
                //Do not need parentModel as we can access childModel by boxModel.childModel.id
