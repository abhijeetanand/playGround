...
$KW.Button = {
    ...
    render: function(buttonModel, context){
		if(!buttonModel.buiskin) buttonModel.buiskin = buttonModel.blockeduiskin;
        var computedSkin = $KW.skins.getWidgetSkinList(buttonModel, context);
		var htmlString = "";
		var skin = buttonModel.skin || "";
		if($KU.isWindowsTablet){
			htmlString = "<button" + $KW.Utils.getBaseHtml(buttonModel, context) + "type='button' class = '" + computedSkin + "'" +  " style='text-align:" + $KW.skins.getContentAlignment(buttonModel) + ";white-space:normal;word-wrap:break-word;" + $KW.skins.getMarginSkin(buttonModel, context) + $KW.skins.getPaddingSkin(buttonModel) + (context.layoutDir && context.ispercent === false ? ";float:" + context.layoutDir : "") + "' kprogressskin='" + skin +"'>" + (buttonModel.displaytext.toString() == "true" ?  buttonModel.text  : "") + "</button>";        
		}
		else{
			htmlString = "<input" + $KW.Utils.getBaseHtml(buttonModel, context) + "type='button' class = '" + computedSkin + "'" + " style='text-align:" + $KW.skins.getContentAlignment(buttonModel) + ";white-space:normal;word-wrap:break-word;" + $KW.skins.getMarginSkin(buttonModel, context) + $KW.skins.getPaddingSkin(buttonModel) + (context.layoutDir && context.ispercent === false ? ";float:" + context.layoutDir : "") + "'" + (buttonModel.displaytext.toString() == "true" ? " value='" + buttonModel.text + "'"  : "") + " kprogressskin='" + skin +"'/>";    
		}
