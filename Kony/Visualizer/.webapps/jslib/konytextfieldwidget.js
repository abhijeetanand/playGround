...
$KW.TextField =
{
    ...
    render : function(textModel, context)
    {
        ...
        var htmlString = "";
		var style = $KW.skins.getMarginSkin(textModel, context) + $KW.skins.getPaddingSkin(textModel);
        ...
        /*if($KU.isAndroid && type == "password" && !$KG.relativeScroll)
		{
			...
			var wrapperDivOpen = $KU.isiPhone ? "<div style='overflow:hidden'>": "";
			var wrapperDivClose = $KU.isiPhone ? "</div>":"";
    ...
    onfocusEventHandler: function(eventObject, target){
		...
		if($KU.isAndroid && target.getAttribute('type') != "number"){
			setTimeout(function (){$KU.setActiveInput(target);},100);
		}
		else{
	...
	adjustHeight: function(formId){
		...
		if( !$KG.relativeScroll && formModel.dockablefooter){
			...
			if(boxModel.containerheightmode == constants.TEXTBOX_CUSTOM_HEIGHT){
				...
				if (boxhtPercent > 0) {
					if(boxModel.containerheightreference == constants.CONTAINER_HEIGHT_BY_PARENT_WIDTH) {
						...
						if( parent && window[boxModel.pf][boxModel.parent].wType == "ScrollBox" )	// Text field inside a ScrollBox
							parent = $KU.getElementById(boxModel.pf+ "_"+ boxModel.parent+ "_parent");
						if(!parent){
				...
				if(kony.appinit.isIE8){
					textBoxes[i].style[line-height] = kht + "px";
				}				
			}
		}
	}
