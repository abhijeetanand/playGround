...
$KW.Map =
{
    ...
    updateView: function(widgetModel, propertyName, propertyValue, oldPropertyValue){
        switch (propertyName) {
			...
			case "containerheightreference":
				if(propertyValue > 0){
					widgetModel.containerheightreference = propertyValue;
				}
				var mapElement = $KU.getNodeByModel(widgetModel);
				if(mapElement)
					mapElement.style.height = $KW.Map.calculateMapHeight(widgetModel)+'px';
				break;
				
			case "containerheight":
				widgetModel.containerheight = propertyValue;
				var mapElement = $KU.getNodeByModel(widgetModel);
				if(mapElement)
					mapElement.style.height = $KW.Map.calculateMapHeight(widgetModel)+'px';
				break;
    ...
    render: function(mapModel, context){
        ...
        if ((mapsrc === "static") || $KU.isBlackBerryNTH || ($KU.isWindowsPhone && $KU.isIE9)) {
			var visibility = (!mapModel.isvisible) ? "hide" : "";
            var mapMode="";
            ...
            var mapdata = eval(mapModel.locationdata || mapModel.address);
            var centrallat = 0;
            var centrallon = 0;
            var markers="";
    ...
    setUpInteractiveCanvasMap: function(){
        ...
        var mapModel = kony.model.getWidgetModel($KW.Map.formID, $KW.Map.mapID);
		mapModel.markers = []; /*clear the markers array before loading condition check for mapModel updated */
    ...
    */
    ...
    getMarkerByIndex: function(index, locationDataList, mapWidget){
        ...
        if (!locationDataList) {
           // var mapWidget = kony.model.getWidgetModel(this.formID, this.mapID);
            var locationDataList = eval(mapWidget.locationdata);
        }
    ...
    ConvertDataMaptoData: function(Model,data){
        var map = Model.locationdata; //widgetDataMapForCallout;
        var masterdata = {};
        ...
        for(var j = 0; j < newkeys.length; j++) 
        {
            ...
            if(value && typeof value != "object" &&  typeof value !== "number" && value.toLowerCase().indexOf("i18n.getlocalizedstring") != -1)
                masterdata[newkeys[j]] = eval(value);
            else
	...
	setMapsHeight: function(formId){
		...
		if(mapElementsBody){
			for(var i=0; i<mapElementsBody.length; i++ ){
				 ...
				 mapElement.style.height = $KW.Map.calculateMapHeight(mapModel)+'px';
				 //alert(temp +" "+ $KU.getMobileWindowSize().height +" "+$KU.getMobileWindowSize().width);
				 
			}
		}
	},
	calculateMapHeight: function(boxModel){
	
		var boxhtPercent = boxModel.containerheight;
		var kht = "";
		if (boxhtPercent > 0) {
			if(boxModel.containerheightreference === constants.MAP_HEIGHT_BY_FORM_REFERENCE || (boxModel.pf== boxModel.parent)) // form height
			{
				//var screenHeight = screen.availHeight; //px
				//kht = Math.round((boxhtPercent * screenHeight) / 100) + "px";
			...
			else if(boxModel.containerheightreference === constants.MAP_HEIGHT_BY_PARENT_WIDTH) // ref to parent width
			{
				
				//var parent = $KU.getElementById(boxModel.pf+ "_"+ boxModel.parent);
				//var parentWidth = (parent && parent.offsetWidth);
				//kht = Math.round((boxhtPercent * parentWidth) / 100) + "px";
