...
$KW.ScrollBox = {
        ...
        switch (propertyName) 
		{
			...
			case "containerheightreference":
				if(propertyValue > 0){
					...
					boxNode && $KW.ScrollBox.adjustDimensionsByNode(widgetModel, boxNode);
				}
				break;
			...
			case "onpush":
				...
				{
					var pullDownEl = document.querySelector("#" + _scrollerId + " #pullDown");
					if(pullDownEl)
						return;

					div.innerHTML = $KU.getPullString();
					scrolleeNode.insertAdjacentElement("afterBegin", div.childNodes[0]);	// pull
				}
				else
				{
					var pullUpEl = document.querySelector("#" + _scrollerId + " #pullUp");
					if(pullUpEl)
						return;
					
					div.innerHTML = $KU.getPushString();
	...
	render: function(widgetModel, context)
	{
		...
		if($KU.isBlackBerryNTH || $KU.isWindowsPhone)
		{
	...
	{        
        ...
        marginpadding += $KW.skins.getMarginSkin(boxModel, context);
        marginpadding += $KW.skins.getPaddingSkin(boxModel);
		...
		//if (scrolldirection != "none")
		{
			...
			if(boxModel.percent == false && boxModel.totalWt < 100) boxModel.totalWt = 100;
            htmlString += "<div id='" + wID + "_scroller' kwidgettype='KScrollBox' name='touchcontainer_KScroller' widgetType='hbox'  swipeDirection='" + scrolldirection + "' class ='kwt100 scrollerX'  >";
            ...
            else 
			{
                 ...
                 */
				 if (boxModel.percent == true) 
				 {
                    ...
                    var containerWt = $KW.skins.getMarAdjustedContainerWeightSkin(childModel, (cellWidth || ""));
	...
	renderVBox : function(boxModel, context, accessAttr)
    {
        ...
        //if (scrolldirection != "none")
		{
            htmlString += "<div id='" + wID + "_scroller' kwidgettype='KScrollBox' name='touchcontainer_KScroller' widgetType='vbox' swipeDirection='" + scrolldirection + "' class ='scrollerX'>";
    ...
    adjustBoxDimensions: function(formId){
		...
		var scrollBoxes = document.querySelectorAll("#" + formId + " div[kwidgettype='ScrollBox'], div[class~='popupmain']  div[kwidgettype='ScrollBox']");
        for (var i = 0; i < scrollBoxes.length; i++) {
			var boxModel = $KU.getModelByNode(scrollBoxes[i]);
			$KW.ScrollBox.adjustDimensionsByNode(boxModel, scrollBoxes[i]);
		}
	},
	...
	adjustDimensionsByNode: function(boxModel, scrollBox){
	   ...
	   // if (scrolldirection != "none" && scrolldirection != "horizontal") 
		...
		if (scrolldirection != "horizontal") 
		{
			...
			{
				if([boxModel.containerheightreference,boxModel.heightreference].contains(constants.SCROLLBOX_HEIGHT_BY_FORM_REFERENCE)) // form height
				{
					// Get form height
				...
				else if([boxModel.containerheightreference,boxModel.heightreference].contains(constants.SCROLLBOX_HEIGHT_BY_PARENT_WIDTH)) // ref to parent width
				{
					//var parent = scroller.parentNode;
					//var parentWidth = parent.offsetWidth;
					var parent = $KU.getElementById(boxModel.pf+ "_"+ boxModel.parent);
				...
				var border = parseInt($KU.getStyle(parentNode, "border-top-width").replace("px", ""), 10) + parseInt($KU.getStyle(parentNode, "border-bottom-width").replace("px", ""), 10);	
				var padding = parseInt($KU.getStyle(parentNode, "padding-top").replace("px", ""), 10) + parseInt($KU.getStyle(parentNode, "padding-bottom").replace("px", ""), 10);	
