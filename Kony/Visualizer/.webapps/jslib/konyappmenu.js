...
$KW.Appmenu =
    ...
    render: function(widgetID, widgetGenerationContext){
        ...
        if(!widgetGenerationContext)
		{
            ...
            if($KU.isTablet || $KU.isMob)
			{
				...
				if($KG["stickyScroll"] || $KG["relativeScroll"] || formModel.dockableappmenu === false)
		...
		/*if(closure){
			if(closure instanceof Function) 
				closure(appmenuWidgetModel);
			else 
				 closure && closure(appmenuWidgetModel);
		} */

		// Close calendar
		var calObj = $KW.Calendar && $KW.Calendar.Popupview.calObj;
		$KW.Calendar && calObj && calObj.dialog.hide();
    },
    ...
    showappmenuitems: function(menulist){
        ...
        else
        {
			...
			if($KG["stickyScroll"] || $KG["relativeScroll"])
				className += " relativePos";
			else
				className += " absolutePos";
