...
if (typeof self !== 'undefined' && typeof self.getTaskName !== 'undefined' && self.getTaskName() + '' === 'jsformatter') {
	var ant = function () {
		...
		for (i = 0; i < mapping.length; i += 1) {
			result = [];
			self.log('Beautifying: ' + mapping[i].dst.toString());
			try {
				result = js_beautify(fileToString(mapping[i].src), {preserve_newlines:false});
			} catch (e) {
				self.fail(e);
			}
